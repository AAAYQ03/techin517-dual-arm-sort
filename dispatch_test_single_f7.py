...
SINGLE_ITEMS = {
    "pen": {
        ...
        "box_xyz": (0.155, -0.058, 0.10),  # f7 自侧右盒 (stationery)
    },
    "battery": {
        "name": "battery",
        ...
        "box_xyz": (0.195, -0.180, 0.12),  # f7 跨侧左盒 (electronics)
    },
    "glue": {
        "name": "glue",
    ...
    "earbuds": {
        ...
        "act_checkpoint": "/home/ubuntu/techin517/outputs/train/act_earbuds_f7/checkpoints/last/pretrained_model",
...
def load_act_policies(item_cfg=None):
    policies = {}
    if item_cfg is not None:
        unique_ckpts = {item_cfg["act_checkpoint"]}
    else:
...
def detect_all_objects(top, processor_cv, model_cv, clip_clf, item_cfg):
    """DINO detect 单物品 (跳过 CLIP)"""
    print("\n  Snapshot from D435i...")
    bgr, depth_raw, intrin = top.read()

    DINO_PROMPT = item_cfg["dino_prompt"]
    DINO_TARGETS = item_cfg["dino_targets"]
    ...
    for d in valid:
        ...
        if not is_valid_position(base_xyz):
            ...
            continue
        d['clip_confidence'] = d['score']  # 用 DINO score 排序
        d['item_config'] = item_config
        bb = tuple(int(x) for x in d['bbox_xyxy'])
        print(f"    bbox={bb} DINO='{d['label']}'({d['score']:.2f}) -> ACCEPT as pen")
        classified.append(d)
...
def main():
    ...
    args = ap.parse_args()
    item_cfg = SINGLE_ITEMS[args.item]

    print("=" * 60)
    print(f"  dispatch_test_single_f7.py - {item_cfg['name']}")
    print(f"  DINO prompt: {item_cfg['dino_prompt']}")
    print(f"  ACT: {item_cfg['act_checkpoint']}")
    print(f"  Box: {item_cfg['box_xyz']}")
    print("=" * 60)
    ...
    policies = load_act_policies(item_cfg)
    print(f"  ✓ {len(policies)} ACT models loaded")

    try:
        clip_clf = None  # 单物体不用 CLIP

        print("\n=== safe_home ===")
        go_to_safe_home(follower)

        print("\n=== CV detect all objects ===")
        targets = detect_all_objects(top, processor_cv, model_cv, clip_clf, item_cfg)
