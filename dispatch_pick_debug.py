...
def run_act(follower, top, wrist, act_bundle, task_name, duration_s):
    ...
    while time.time() - t_start < duration_s:
        ...
        if n % 30 == 0:
            print(f"    [t={time.time()-t_start:.1f}s n={n}] state.gripper={state[5]:.1f} action.gripper={action[5]:.1f}")
...
def detect_all_objects(top, processor_cv, model_cv, clip_clf):
    ...
    DINO_PROMPT = "a battery. a usb stick."
    DINO_TARGETS = {"battery", "usb stick", "usb"}
    ...
    for d in valid:
        ...
        print(f"    bbox={bb} DINO='{d['label']}'({d['score']:.2f}) -> CLIP='{clip_label}'({clip_conf:.2f})")
