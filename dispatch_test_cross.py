...
def run_act(follower, top, wrist, act_bundle, task_name, duration_s):
    ...
    while time.time() - t_start < duration_s:
        ...
        if n == 0:
            cv2.imwrite("/home/ubuntu/techin517/diag_images/cross_act_t0_wrist.jpg", cv2.cvtColor(wrist_rgb, cv2.COLOR_RGB2BGR))
            cv2.imwrite("/home/ubuntu/techin517/diag_images/cross_act_t0_top.jpg", cv2.cvtColor(top_rgb, cv2.COLOR_RGB2BGR))
            print(f"    [DEBUG] saved cross_act_t0 frames")
        n += 1
        el = time.time() - t_loop
...
def detect_all_objects(top, processor_cv, model_cv, clip_clf):
    """DINO 一次 detect + CLIP 二次分类"""
    print("\n  Snapshot from D435i...")
    bgr, depth_raw, intrin = top.read()

    DINO_PROMPT = "a battery. a usb stick. a black pen."
    DINO_TARGETS = {"battery", "usb stick", "usb", "black pen", "pen", "marker"}
    print(f"  DINO detect: {DINO_PROMPT!r}")
    dets = detect_objects(
    ...
    item_by_clip = {it['clip_label']: it for it in ITEMS}
    classified = []
    for d in valid:
        ...
        if not is_valid_position(base_xyz):
            ...
            continue
        # === Shape prior + CLIP 二阶段分类 ===
        # 5/21 PDF: marker pen aspect 6.7:1, battery 1.7:1, USB 2.7:1
        ...
        if aspect > 4.0:
            d['clip_label'] = 'marker pen'
            d['clip_confidence'] = d['score']
            print(f"    bbox={bb} aspect={aspect:.2f} DINO='{d['label']}'({d['score']:.2f}) -> SHAPE PRIOR: pen")
            if 'marker pen' in item_by_clip:
                d['item_config'] = item_by_clip['marker pen']
                classified.append(d)
            else:
                print(f"      [REJECT] no pen config")
        else:
            clip_label, clip_conf, _ = clip_clf.classify(bgr, d['bbox_xyxy'])
            d['clip_label'] = clip_label
            d['clip_confidence'] = clip_conf
            print(f"    bbox={bb} aspect={aspect:.2f} DINO='{d['label']}'({d['score']:.2f}) -> CLIP='{clip_label}'({clip_conf:.2f})")
            if clip_label in item_by_clip:
                d['item_config'] = item_by_clip[clip_label]
                classified.append(d)
            else:
                print(f"      [REJECT] CLIP says not a target")

    def bbox_center(d):
        x1, y1, x2, y2 = d['bbox_xyxy']
        return ((x1+x2)/2, (y1+y2)/2)
    deduped = []
    sorted_dets = sorted(classified, key=lambda d: -d['clip_confidence'])
...
def main():
    print("=" * 60)
    print("  dispatch_pick.py v2 - 分开 detect + 位置过滤")
    print("=" * 60)
    ...
    try:
        ...
        targets = detect_all_objects(top, processor_cv, model_cv, clip_clf)
        # 按 base_xyz[0] 降序: 远的物体先抓 (避免前方 gripper 抓近物时撞远物)
        targets.sort(key=lambda d: -d['base_xyz'][0])
        if not targets:
