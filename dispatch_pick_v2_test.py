...
def detect_all_objects(top, processor_cv, model_cv):
    """每个 ITEM 单独 detect, 应用位置过滤, 去重"""
    print("\n  Snapshot from D435i...")
    bgr, depth_raw, intrin = top.read()
    
    all_detections = []
    for item in ITEMS:
        print(f"\n  Detect prompt: {item['prompt']!r}")
        dets = detect_objects(
            color_image=bgr, depth_image=depth_raw, intrin=intrin,
            depth_scale=DEPTH_SCALE, processor=processor_cv, model=model_cv,
            text_prompt=item['prompt'], target_classes=item['target_classes'],
        ...
        for d in valid:
            ...
            d['base_xyz'] = base_xyz
            d['item_config'] = item  # 强制绑定 prompt 对应的 ACT
            if is_valid_position(base_xyz):
                in_workspace.append(d)
                print(f"    ✓ '{d['label']}' score={d['score']:.2f} base_xyz={base_xyz}")
            else:
                print(f"    ✗ filtered '{d['label']}' base_xyz={base_xyz} (out of workspace)")
        all_detections.extend(in_workspace)
    ...
    for d in sorted_dets:
        ...
        for k in deduped:
            kx, ky = bbox_center(k)
            if (cx-kx)**2 + (cy-ky)**2 < 60**2:  # 60 像素阈值
                too_close = True
                break
        if not too_close:
            deduped.append(d)
...
def main():
    ...
    try:
        ...
        targets = detect_all_objects(top, processor_cv, model_cv)
        if not targets:
